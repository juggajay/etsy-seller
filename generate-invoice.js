...
function roundedRect(page, x, y, w, h, color) {
  // Simulate rounded corners with overlapping rects
  const r = 3;
  rect(page, x + r, y, w - 2 * r, h, color);
  rect(page, x, y + r, w, h - 2 * r, color);
  // Corner fills (small squares — close enough at PDF scale)
...
// ============================================================
// PAGE 1: HOW TO USE — polished version
// ============================================================
function buildHowToUsePage(page, c, fontR, fontM, fontB) {
  ...
  // Header band with layered depth
  ...
  const instructions = [
    ...
    { title: 'Auto-Calculating Fields', body: 'Enter the Quantity and Rate for each line item — the Amount calculates automatically. Subtotal, Tax, and Total update in real time. Zero manual math.' },
    ...
    { title: 'Customise Your Details', body: 'Fill in your business name, address, payment details, and terms. Save a master copy with your details pre-filled so you only need to update client info for each new invoice.' },
    { title: 'Save & Send', body: 'After filling out the invoice, go to File > Save As to keep your completed copy. Email it directly to your client as a professional PDF attachment.' },
    { title: 'Multiple Formats Included', body: 'This download includes: Fillable PDF (this file) for quick professional invoices, Google Sheets version with built-in formulas, and a Canva template link for full design customisation.' },
  ];
...
// ============================================================
// PAGE 2: INVOICE — polished version
// ============================================================
function buildInvoicePage(page, form, c, fontR, fontM, fontB, fontL) {
  const cw = W - M * 2;
  ...
  // Layered accent for depth
  rect(page, 0, H - 95, W * 0.55, 95, c.accent, { opacity: 0.2 });
  // Bold geometric block — top right
  rect(page, W - 170, H - 95, 170, 32, c.highlight);

  txt(page, 'INVOICE', M, H - 58, fontB, 32, c.white);

  // Logo area — subtle outlined box
  const logoX = W - M - 105;
  const logoY = H - 86;
  ...
  const metaItems = [
    { label: 'INVOICE NO.', name: 'invoice_number', default: 'INV-001', x: M + 12 },
    { label: 'DATE', name: 'invoice_date', default: '', x: M + 185 },
    { label: 'DUE DATE', name: 'due_date', default: '', x: M + 355 },
  ];
  ...
  // BILL TO
  const toX = M + colW + 20;
  roundedRect(page, toX, secY - 2, 60, 16, c.highlight);
  txt(page, 'BILL TO', toX + 10, secY + 1, fontM, 8, c.white);
  ...
  // Discount
  txt(page, 'Discount', tLabelX + 12, tY + 4, fontR, 9, c.text);
  const discF = form.createTextField('discount');
  discF.addToPage(page, {
    x: tFieldX, y: tY - 2, width: tFieldW, height: 18,
    borderWidth: 0.5, borderColor: c.border, backgroundColor: rgb(1, 1, 1),
  });
  discF.setAlignment(2); discF.setFontSize(9); discF.setText('0.00');
  ...
  txt(page, 'TOTAL', tLabelX + 12, tY + 4, fontB, 12, c.white);
  const totalF = form.createTextField('total');
  totalF.addToPage(page, {
    x: tFieldX, y: tY - 1, width: tFieldW, height: 20,
    borderWidth: 0, backgroundColor: c.primary, textColor: c.white,
  });
  totalF.setAlignment(2); totalF.setFontSize(11); totalF.enableReadOnly();
  calcFieldRefs.push({ name: 'total', field: totalF });
  ...
  txt(page, 'PAYMENT TERMS', M + 14, bY + 28, fontM, 7, c.primary);
  const ptF = form.createTextField('payment_terms');
  ptF.addToPage(page, {
    x: M + 14, y: bY - 2, width: halfW - 14, height: 26,
    borderWidth: 0.5, borderColor: c.border, backgroundColor: rgb(1, 1, 1),
  });
  ptF.setFontSize(8); ptF.setText('Payment due within 30 days of invoice date.');
  ...
  nF.setFontSize(8); nF.setText('Thank you for your business!');

  // Bank details
  const bankY = bY - 42;
  roundedRect(page, M, bankY - 2, cw, 34, c.light);
  txt(page, 'BANK / PAYMENT DETAILS', M + 10, bankY + 20, fontM, 7, c.primary);
  ...
  txt(page, 'Professional Invoice Template', M, 7, fontR, 7, c.white, { opacity: 0.6 });
...
function buildTermsPage(page, c, fontR, fontM, fontB) {
  ...
  const terms = [
    { t: '1. Payment Terms', b: 'Payment is due within the timeframe specified on the invoice. Late payments may be subject to a late fee of 1.5% per month on the outstanding balance. All amounts are in the currency specified on the invoice.' },
    { t: '2. Scope of Work', b: 'This invoice covers the services or products described in the line items above. Any additional work or changes to the original scope will be billed separately and require written agreement from both parties.' },
    { t: '3. Revisions & Changes', b: 'Any revisions or changes to the delivered work beyond the agreed scope may incur additional charges. The client will be notified of any additional costs before work begins.' },
    { t: '4. Intellectual Property', b: 'Upon full payment, the client receives full rights to the deliverables as specified in the project agreement. The service provider retains the right to display the work in their portfolio unless otherwise agreed.' },
    { t: '5. Confidentiality', b: 'Both parties agree to keep confidential any proprietary information shared during the course of the project. This obligation survives the completion of the project.' },
    { t: '6. Limitation of Liability', b: 'The total liability shall not exceed the amount paid for the services described in this invoice. Neither party shall be liable for indirect, incidental, or consequential damages.' },
    { t: '7. Cancellation', b: 'Either party may cancel the project with written notice. In the event of cancellation, the client is responsible for payment of all work completed up to the date of cancellation.' },
    { t: '8. Dispute Resolution', b: 'Any disputes arising from this invoice or related services shall first be addressed through good-faith negotiation. If unresolved, disputes shall be settled through mediation or arbitration.' },
  ];

  const maxW = W - M * 2 - 16;

  for (const term of terms) {
    // Term title with accent dot
...
function buildThankYouPage(page, c, fontR, fontM, fontB, fontL) {
  ...
  // Geometric accents — bold contemporary
  ...
  txt(page, 'for your business', M + 28, H - 222, fontL, 17, c.white, { opacity: 0.75 });

  // Message
  const msgY = H - 355;
  txt(page, 'We truly appreciate your trust and partnership.', M + 24, msgY, fontR, 14, c.text);
  txt(page, 'If you have any questions about this invoice,', M + 24, msgY - 30, fontR, 11.5, c.muted);
  txt(page, 'please don\'t hesitate to reach out.', M + 24, msgY - 48, fontR, 11.5, c.muted);
...
function addCalculationJS(pdfDoc, form, calcFieldRefs) {
  ...
  // Total
  const totE = calcFieldRefs.find(f => f.name === 'total');
  if (totE) {
    const js = 'var s=Number(this.getField("subtotal").value)||0;var t=Number(this.getField("tax_amount").value)||0;var d=Number(this.getField("discount").value)||0;var total=s+t-d;event.value=total>0?"$"+total.toFixed(2):""';
...
// ============================================================
// MAIN — Split-and-merge: Invoice page built WITHOUT fontkit (forms work),
//        design pages built WITH fontkit (Inter font), then merged.
// ============================================================
async function generateInvoice(schemeName) {
  ...
  buildHowToUsePage(designDoc.addPage([W, H]), colors, dR, dM, dB);         // design page 0
  buildTermsPage(designDoc.addPage([W, H]), colors, dR, dM, dB);            // design page 1
  buildThankYouPage(designDoc.addPage([W, H]), colors, dR, dM, dB, dL);     // design page 2

  const designBytes = await designDoc.save();

  // ---- DOC B: Invoice page (NO fontkit — StandardFonts only) ----
  const mainDoc = await PDFDocument.create();
  // NO fontkit registration — keeps AcroForm clean for Adobe JS
  ...
  mainDoc.setTitle(`Professional Invoice Template — ${colors.name}`);
  mainDoc.setAuthor('Small Business Templates');
  mainDoc.setSubject('Fillable Invoice Template with Auto-Calculating Fields');
  mainDoc.setKeywords(['invoice', 'template', 'fillable', 'business', 'professional']);
  mainDoc.setCreator('Small Business Templates');

  const invoicePage = mainDoc.addPage([W, H]);
  const form = mainDoc.getForm();
  const calcRefs = buildInvoicePage(invoicePage, form, colors, fontR, fontM, fontB, fontL);

  form.updateFieldAppearances(fontR);
  addCalculationJS(mainDoc, form, calcRefs);

  // NeedAppearances so Adobe recalculates field displays
  ...
  // ---- MERGE: Copy design pages into mainDoc around the invoice ----
  ...
  // Insert How to Use BEFORE invoice (index 0)
  mainDoc.insertPage(0, howToUse);
  // Append Terms and Thank You AFTER invoice
