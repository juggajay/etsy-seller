...
async function generateQuoteSheet(schemeName) {
  ...
  const ws = wb.addWorksheet('Quote', {
    ...
    headerFooter: { oddFooter: 'Professional Quote Template' },
  });
  ...
  ws.getCell('F5').value = 'Valid Until';
  ...
  ws.getCell(footerStart, 2).value = 'VALIDITY & TERMS';
  ws.getCell(footerStart, 2).font = { size: 8, bold: true, color: { argb: c.primary } };

  ws.mergeCells(`B${footerStart + 1}:C${footerStart + 2}`);
  ws.getCell(footerStart + 1, 2).value = 'This quote is valid for 30 days from the date issued.';
  ...
  ws.getCell(footerStart + 1, 5).value = 'Thank you for considering our services!';
  ...
  ws.getCell(footerBarRow, 2).value = 'Professional Quote Template';
  ...
  const filename = `Quote-Template-${schemeLabel === 'Forest' ? 'Forest-Green' : schemeLabel}.xlsx`;
...
async function main() {
  console.log('Generating Quote Spreadsheets...\n');

  for (const scheme of ['navy', 'forest', 'terracotta', 'charcoal']) {
    await generateQuoteSheet(scheme);
  }

  console.log('\nDone! Quote spreadsheets generated in ./output/');
}
