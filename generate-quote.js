...
// ============================================================
// COLOR SCHEMES — 4 variants (same as invoice)
// ============================================================
const COLOR_SCHEMES = {
  navy: {
    name: 'Navy',
...
function buildHowToUsePage(page, c, fontR, fontM, fontB) {
  ...
  const instructions = [
    ...
    { title: 'Auto-Calculating Fields', body: 'Enter the Quantity and Unit Price for each line item — the Amount calculates automatically. Subtotal, Tax, and Total update in real time. Zero manual math required.' },
    ...
    { title: 'Customise Your Details', body: 'Fill in your business name, address, and contact details. Save a master copy with your details pre-filled so you only need to update client info for each new quote.' },
    { title: 'Save & Send', body: 'After filling out the quote, go to File > Save As to keep your completed copy. Email it directly to your client as a professional PDF attachment.' },
    { title: 'Multiple Formats Included', body: 'This download includes: Fillable PDF (this file) for quick professional quotes, Google Sheets version with built-in formulas, and a Word template for full design customisation.' },
  ];
...
// ============================================================
// PAGE 2: QUOTE / ESTIMATE
// ============================================================
function buildQuotePage(page, form, c, fontR, fontM, fontB, fontL) {
  const cw = W - M * 2;
  ...
  txt(page, 'QUOTE', M, H - 58, fontB, 32, c.white);
  ...
  const metaItems = [
    { label: 'QUOTE NO.', name: 'quote_number', default: 'QTE-001', x: M + 12 },
    { label: 'DATE', name: 'quote_date', default: '', x: M + 185 },
    { label: 'VALID UNTIL', name: 'valid_until', default: '', x: M + 355 },
  ];
  ...
  // QUOTE FOR
  const toX = M + colW + 20;
  roundedRect(page, toX, secY - 2, 80, 16, c.highlight);
  txt(page, 'QUOTE FOR', toX + 10, secY + 1, fontM, 8, c.white);
  ...
  // Validity / acceptance terms
  rect(page, M, bY - 2, 4, 42, c.highlight);
  txt(page, 'VALIDITY & TERMS', M + 14, bY + 28, fontM, 7, c.primary);
  const ptF = form.createTextField('validity_terms');
  ptF.addToPage(page, {
    x: M + 14, y: bY - 2, width: halfW - 14, height: 26,
    borderWidth: 0.5, borderColor: c.border, backgroundColor: rgb(1, 1, 1),
  });
  ptF.setFontSize(8); ptF.setText('This quote is valid for 30 days from the date issued.');
  ...
  nF.setFontSize(8); nF.setText('Thank you for considering our services!');
  ...
  txt(page, 'Professional Quote Template', M, 7, fontR, 7, c.white, { opacity: 0.6 });
...
function buildTermsPage(page, c, fontR, fontM, fontB) {
  ...
  const terms = [
    { t: '1. Quote Validity', b: 'This quote is valid for the period specified on the quote page. After the validity period, prices and availability may change without notice. A new quote may be required for work commencing after expiry.' },
    { t: '2. Scope of Work', b: 'This quote covers the services or products described in the line items above. Any additional work or changes to the original scope will require a revised quote and written agreement from both parties.' },
    { t: '3. Acceptance', b: 'To accept this quote, please sign and return a copy or confirm acceptance via email. Acceptance of this quote constitutes agreement to the terms and conditions outlined herein.' },
    { t: '4. Payment Terms', b: 'Unless otherwise specified, a deposit of 50% is required upon acceptance, with the balance due upon completion. Payment terms may vary — specific terms will be confirmed upon acceptance.' },
    { t: '5. Revisions & Changes', b: 'Any revisions or changes to the quoted scope of work may result in adjusted pricing. The client will be notified of any cost changes before additional work begins.' },
    { t: '6. Cancellation', b: 'Either party may cancel the engagement with written notice. In the event of cancellation after acceptance, the client is responsible for payment of all work completed up to the cancellation date.' },
    { t: '7. Intellectual Property', b: 'Upon full payment, the client receives full rights to the deliverables as specified in the project agreement. The service provider retains the right to display the work in their portfolio unless otherwise agreed.' },
    { t: '8. Limitation of Liability', b: 'The total liability shall not exceed the amount quoted for the services described. Neither party shall be liable for indirect, incidental, or consequential damages arising from this quote or related work.' },
  ];
...
function buildThankYouPage(page, c, fontR, fontM, fontB, fontL) {
  ...
  txt(page, 'for considering our services', M + 28, H - 222, fontL, 15, c.white, { opacity: 0.75 });

  const msgY = H - 355;
  txt(page, 'We appreciate the opportunity to provide this quote.', M + 24, msgY, fontR, 14, c.text);
  txt(page, 'If you have any questions or would like to proceed,', M + 24, msgY - 30, fontR, 11.5, c.muted);
...
// MAIN
// ============================================================
async function generateQuote(schemeName) {
  ...
  // ---- DOC B: Quote page (NO fontkit — StandardFonts only for AcroForm) ----
  ...
  mainDoc.setTitle(`Professional Quote Template — ${colors.name}`);
  mainDoc.setAuthor('Small Business Templates');
  mainDoc.setSubject('Fillable Quote Template with Auto-Calculating Fields');
  mainDoc.setKeywords(['quote', 'estimate', 'template', 'fillable', 'business', 'professional']);
  mainDoc.setCreator('Small Business Templates');

  const quotePage = mainDoc.addPage([W, H]);
  const form = mainDoc.getForm();
  const calcRefs = buildQuotePage(quotePage, form, colors, fontR, fontM, fontB, fontL);
