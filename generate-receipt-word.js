...
function generateReceiptDoc(schemeName) {
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({
              children: [new TextRun({ text: 'RECEIPT', bold: true, color: 'FFFFFF', size: 52, font: 'Calibri' })],
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'RECEIPT NO.', bold: true, color: c.muted, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: 'REC-001', size: 18, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
          ...
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'DATE OF PAYMENT', bold: true, color: c.muted, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
          ...
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'PAYMENT METHOD', bold: true, color: c.muted, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: '_______________', size: 18, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            shading: { type: ShadingType.SOLID, color: c.light },
            width: { size: 33, type: WidthType.PERCENTAGE },
  ...
  // ---- FROM / RECEIPT FOR ----
  const fromLabels = ['Company / Name', 'Address', 'Email', 'Phone'];
  const toLabels = ['Client / Company', 'Address', 'Email', 'Phone'];

  function contactSection(label, bgColor, labels) {
    const rows = [
      new TableRow({
        children: [
          new TableCell({
  ...
  const fromToTable = new Table({
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [contactSection('FROM', c.primary, fromLabels)],
          ...
          new TableCell({
            children: [contactSection('RECEIPT FOR', c.highlight, toLabels)],
            width: { size: 48, type: WidthType.PERCENTAGE },
            borders: noBorder,
          }),
        ],
  ...
  // ---- TOTALS ----
  function totalRow(label, value, isFinal, isHighlight) {
    const bgColor = isFinal ? c.primary : isHighlight ? c.highlight : undefined;
    const textColor = (isFinal || isHighlight) ? 'FFFFFF' : c.text;
    const fontSize = isFinal ? 22 : isHighlight ? 20 : 18;
    const isBold = isFinal || isHighlight;
      ...
      children: [
        ...
        new TableCell({
          children: [new Paragraph({
            ...
            spacing: { before: isBold ? 60 : 30, after: isBold ? 60 : 30 },
          })],
        ...
        new TableCell({
          ...
          shading: bgColor
            ...
            : { type: ShadingType.SOLID, color: c.light },
          width: { size: 22, type: WidthType.PERCENTAGE },
          borders: noBorder,
          verticalAlign: VerticalAlign.CENTER,
        }),
  ...
  const totalsTable = new Table({
    rows: [
      totalRow('Subtotal', '$________', false, false),
      totalRow('Tax (___%)', '$________', false, false),
      totalRow('TOTAL PAID', '$________', true, false),
      totalRow('Amount Tendered', '$________', false, false),
      totalRow('CHANGE DUE', '$________', false, true),
    ],
    width: { size: 100, type: WidthType.PERCENTAGE },
    layout: TableLayoutType.FIXED,
  });
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'RECEIPT TERMS', bold: true, color: c.primary, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: 'This receipt confirms payment in full for the items/services listed above.', size: 16, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            width: { size: 48, type: WidthType.PERCENTAGE },
          ...
          new TableCell({
            children: [
              ...
              new Paragraph({ children: [new TextRun({ text: 'Thank you for your payment!', size: 16, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            width: { size: 48, type: WidthType.PERCENTAGE },
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'PAYMENT DETAILS', bold: true, color: c.primary, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({
              children: [new TextRun({ text: 'Professional Receipt Template', color: 'FFFFFF', size: 14, font: 'Calibri' })],
              spacing: { before: 30, after: 30 },
              indent: { left: 200 },
            })],
            shading: { type: ShadingType.SOLID, color: c.primary },
  ...
  const doc = new Document({
    ...
    title: `Professional Receipt Template — ${c.name}`,
    description: 'Editable Receipt Template for Small Business',
    sections: [{
      properties: {
        page: {
...
async function main() {
  console.log('Generating Word Receipt Templates...\n');
  ...
  for (const scheme of ['navy', 'forest', 'terracotta', 'charcoal']) {
    ...
    const doc = generateReceiptDoc(scheme);
    const buffer = await Packer.toBuffer(doc);
    const filename = `Receipt-Template-${c.name.replace(/\s+/g, '-')}.docx`;
  ...
  console.log('\nDone! Word receipt templates saved to ./output/');
}
