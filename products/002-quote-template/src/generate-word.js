...
function generateQuoteDoc(schemeName) {
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({
              children: [new TextRun({ text: 'QUOTE', bold: true, color: 'FFFFFF', size: 52, font: 'Calibri' })],
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'QUOTE NO.', bold: true, color: c.muted, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: 'QTE-001', size: 18, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
          ...
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'VALID UNTIL', bold: true, color: c.muted, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: '_______________', size: 18, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            shading: { type: ShadingType.SOLID, color: c.light },
            width: { size: 33, type: WidthType.PERCENTAGE },
    ...
    rows: [
      new TableRow({
        children: [
          ...
          new TableCell({
            children: [contactSection('QUOTE FOR', c.highlight, toLabels)],
            width: { size: 48, type: WidthType.PERCENTAGE },
            borders: noBorder,
          }),
        ],
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [
              new Paragraph({ children: [new TextRun({ text: 'VALIDITY & TERMS', bold: true, color: c.primary, size: 14, font: 'Calibri' })], spacing: { before: 60 } }),
              new Paragraph({ children: [new TextRun({ text: 'This quote is valid for 30 days from the date issued.', size: 16, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            width: { size: 48, type: WidthType.PERCENTAGE },
          ...
          new TableCell({
            children: [
              ...
              new Paragraph({ children: [new TextRun({ text: 'Thank you for considering our services!', size: 16, color: c.text, font: 'Calibri' })], spacing: { after: 60 } }),
            ],
            width: { size: 48, type: WidthType.PERCENTAGE },
    ...
    rows: [
      new TableRow({
        children: [
          new TableCell({
            children: [new Paragraph({
              children: [new TextRun({ text: 'Professional Quote Template', color: 'FFFFFF', size: 14, font: 'Calibri' })],
              spacing: { before: 30, after: 30 },
              indent: { left: 200 },
            })],
            shading: { type: ShadingType.SOLID, color: c.primary },
  ...
  const doc = new Document({
    ...
    title: `Professional Quote Template — ${c.name}`,
    description: 'Editable Quote Template for Small Business',
    sections: [{
      properties: {
        page: {
...
async function main() {
  console.log('Generating Word Quote Templates...\n');
  ...
  for (const scheme of ['navy', 'forest', 'terracotta', 'charcoal']) {
    ...
    const doc = generateQuoteDoc(scheme);
    const buffer = await Packer.toBuffer(doc);
    const filename = `Quote-Template-${c.name.replace(/\s+/g, '-')}.docx`;
  ...
  console.log('\nDone! Word quote templates saved to ./output/');
}
