// =============================================================
// generate-construction-v2.js — N1 Construction Invoice Generator
...
const { CONSTRUCTION_COLOURS, toCssVars } = require('./lib/construction-colours');
const { CONSTRUCTION_FIELD_DEFS } = require('./lib/construction-field-definitions');
const { extractFieldCoordinates, PAGE_W, PAGE_H } = require('./lib/render-pdf');

const TEMPLATES_DIR = path.join(__dirname, 'templates');
const OUTPUT_DIR = path.join(__dirname, 'output', 'niche-invoices', 'N1-construction');

// Construction-specific CSS additions
...
/* Section Labels (LABOUR / MATERIALS) */
.section-label-row {
  margin-bottom: 0;
}
...
/* Compact table rows for construction */
.compact-table {
  margin-bottom: 0;
}
...
/* Bottom grid: progress claim + totals side by side */
.bottom-grid {
  display: grid;
...
/**
 * Build the full HTML document for construction template
...
function buildDocument(colourScheme) {
  ...
  const howToUse = readPageContent('construction-how-to-use.html');
  const invoice = readPageContent('construction-invoice.html');
  const quote = readPageContent('construction-quote.html');
...
async function main() {
  ...
  const allFields = [
    ...CONSTRUCTION_FIELD_DEFS.invoice.fields,
    ...CONSTRUCTION_FIELD_DEFS.quote.fields,
  ];
  const allCO = [
    ...CONSTRUCTION_FIELD_DEFS.invoice.co,
    ...CONSTRUCTION_FIELD_DEFS.quote.co,
  ];

  const schemes = Object.keys(CONSTRUCTION_COLOURS);

  console.log(`\n  Construction Invoice Generator v2`);
  console.log(`  ──────────────────────────────────`);
  ...
  try {
    for (const schemeName of schemes) {
      const scheme = CONSTRUCTION_COLOURS[schemeName];
      ...
      // Write output
      const filename = `Construction-Invoice-${label}.pdf`;
  ...
  const zipPath = path.join(OUTPUT_DIR, 'Construction-Invoice-Bundle.zip');
