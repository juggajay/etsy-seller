// =============================================================
// generate-contractor-v2.js — N5 Contractor Invoice Generator
...
const { CONTRACTOR_COLOURS, toCssVars } = require('./lib/contractor-colours');
const { CONTRACTOR_FIELD_DEFS } = require('./lib/contractor-field-definitions');
const { extractFieldCoordinates, PAGE_W, PAGE_H } = require('./lib/render-pdf');

const TEMPLATES_DIR = path.join(__dirname, 'templates');
const OUTPUT_DIR = path.join(__dirname, 'output', 'niche-invoices', 'N5-contractor');

// Contractor-specific CSS additions
const CONTRACTOR_CSS = `
...
/* Bottom grid: markup/fees + totals side by side */
.bottom-grid {
  display: grid;
...
/**
 * Build the full HTML document for contractor template
...
function buildDocument(colourScheme) {
  ...
  const howToUse = readPageContent('contractor-how-to-use.html');
  const invoice = readPageContent('contractor-invoice.html');
  const quote = readPageContent('contractor-quote.html');
...
async function main() {
  ...
  const allFields = [
    ...CONTRACTOR_FIELD_DEFS.invoice.fields,
    ...CONTRACTOR_FIELD_DEFS.quote.fields,
  ];
  const allCO = [
    ...CONTRACTOR_FIELD_DEFS.invoice.co,
    ...CONTRACTOR_FIELD_DEFS.quote.co,
  ];

  const schemes = Object.keys(CONTRACTOR_COLOURS);

  console.log(`\n  Contractor Invoice Generator v2`);
  console.log(`  ────────────────────────────────────────`);
  ...
  try {
    for (const schemeName of schemes) {
      const scheme = CONTRACTOR_COLOURS[schemeName];
      ...
      // Write output
      const filename = `Contractor-Invoice-${label}.pdf`;
  ...
  const zipPath = path.join(OUTPUT_DIR, 'Contractor-Invoice-Bundle.zip');
