// =============================================================
// generate-freelancer-v2.js — N3 Freelancer Invoice Generator
...
const { FREELANCER_COLOURS, toCssVars } = require('./lib/freelancer-colours');
const { FREELANCER_FIELD_DEFS } = require('./lib/freelancer-field-definitions');
const { extractFieldCoordinates, PAGE_W, PAGE_H } = require('./lib/render-pdf');

const TEMPLATES_DIR = path.join(__dirname, 'templates');
const OUTPUT_DIR = path.join(__dirname, 'output', 'niche-invoices', 'N3-freelancer');

// Freelancer-specific CSS additions
const FREELANCER_CSS = `
...
/* Bottom grid: scope + totals side by side */
.bottom-grid {
  display: grid;
...
/* Project Details Box (reuses progress-claim styles) */
.progress-claim-box {
...
/**
 * Build the full HTML document for freelancer template
...
function buildDocument(colourScheme) {
  ...
  const howToUse = readPageContent('freelancer-how-to-use.html');
  const invoice = readPageContent('freelancer-invoice.html');
  const quote = readPageContent('freelancer-quote.html');
...
async function main() {
  ...
  const allFields = [
    ...FREELANCER_FIELD_DEFS.invoice.fields,
    ...FREELANCER_FIELD_DEFS.quote.fields,
  ];
  const allCO = [
    ...FREELANCER_FIELD_DEFS.invoice.co,
    ...FREELANCER_FIELD_DEFS.quote.co,
  ];

  const schemes = Object.keys(FREELANCER_COLOURS);

  console.log(`\n  Freelancer Invoice Generator v2`);
  console.log(`  ────────────────────────────────────────`);
  ...
  try {
    for (const schemeName of schemes) {
      ...
      // Write output
      const filename = `Freelancer-Invoice-${label}.pdf`;
  ...
  const zipPath = path.join(OUTPUT_DIR, 'Freelancer-Invoice-Bundle.zip');
