// =============================================================
// generate-photography-v2.js — N4 Photography Invoice Generator
...
const { PHOTOGRAPHY_COLOURS, toCssVars } = require('./lib/photography-colours');
const { PHOTOGRAPHY_FIELD_DEFS } = require('./lib/photography-field-definitions');
const { extractFieldCoordinates, PAGE_W, PAGE_H } = require('./lib/render-pdf');

const TEMPLATES_DIR = path.join(__dirname, 'templates');
const OUTPUT_DIR = path.join(__dirname, 'output', 'niche-invoices', 'N4-photography');

// Photography-specific CSS additions
const PHOTOGRAPHY_CSS = `
...
/* Bottom grid: deposit + totals side by side */
.bottom-grid {
  display: grid;
...
/**
 * Build the full HTML document for photography template
...
function buildDocument(colourScheme) {
  ...
  const howToUse = readPageContent('photography-how-to-use.html');
  const invoice = readPageContent('photography-invoice.html');
  const quote = readPageContent('photography-quote.html');
...
async function main() {
  ...
  const allFields = [
    ...PHOTOGRAPHY_FIELD_DEFS.invoice.fields,
    ...PHOTOGRAPHY_FIELD_DEFS.quote.fields,
  ];
  const allCO = [
    ...PHOTOGRAPHY_FIELD_DEFS.invoice.co,
    ...PHOTOGRAPHY_FIELD_DEFS.quote.co,
  ];

  const schemes = Object.keys(PHOTOGRAPHY_COLOURS);

  console.log(`\n  Photography Invoice Generator v2`);
  console.log(`  ────────────────────────────────────────`);
  ...
  try {
    for (const schemeName of schemes) {
      const scheme = PHOTOGRAPHY_COLOURS[schemeName];
      ...
      // Write output
      const filename = `Photography-Invoice-${label}.pdf`;
  ...
  const zipPath = path.join(OUTPUT_DIR, 'Photography-Invoice-Bundle.zip');
