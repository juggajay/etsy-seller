// =============================================================
// generate-receipt-v2.js — Receipt PDF Generator (v2 pipeline)
// =============================================================
...
async function main() {
  ...
  const { fields, co } = FIELD_DEFS.receipt;
  const schemes = Object.keys(COLOUR_SCHEMES);

  console.log(`\n  Receipt Template Generator v2`);
  console.log(`  ─────────────────────────────`);
  ...
  try {
    for (const schemeName of schemes) {
      ...
      const filename = `Receipt-Template-${label.replace(/\s+/g, '-')}.pdf`;
  ...
  console.log(`\n  All receipt PDFs saved to: ${OUTPUT_DIR}\n`);
}
